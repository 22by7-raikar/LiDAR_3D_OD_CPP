...
/* Ray struct for lidar sensor
   Represents a single ray cast from the lidar sensor
   The ray is cast from the lidar sensor position in a specific direction
   and checks for collisions with the ground slope and cars in the scene.
   If a collision is detected, the ray stops and the point is added to the cloud.
   The ray also adds noise to the point based on the standard deviation error.
*/
struct Ray
{
...
struct Lidar
{
	...
	Lidar(std::vector<Car> setCars, double setGroundSlope)
		: cloud(std::make_shared<pcl::PointCloud<pcl::PointXYZ>>()), position(0,0,2.6)
	{
