...
void simpleHighway(pcl::visualization::PCLVisualizer::Ptr& viewer, bool render_clusters = true, bool render_boxes = true)
{
    // ----------------------------------------------------
    ...
    // Done:: Initialize LiDAR sensor 
    std::unique_ptr<Lidar> lidar(new Lidar(cars, 0.0)); //0.0 slope angle
    // Lidar* lidar = new Lidar(cars, 0.0); //0.0 slope angle
    ...
    // Done:: Point processor Initialization
    ...
    std::pair<pcl::PointCloud<pcl::PointXYZ>::Ptr, pcl::PointCloud<pcl::PointXYZ>::Ptr> segmentCloud = 
        pointProcessor->SegmentPlane(input_cloud, 100, 0.2);
...
int main (int argc, char** argv)
{
    ...
    std::unique_ptr<ProcessPointClouds<pcl::PointXYZI>> pointProcessorI_ptr(new ProcessPointClouds<pcl::PointXYZI>());
    std::vector<boost::filesystem::path> stream = pointProcessorI_ptr->streamPcd("../src/sensors/data/pcd/data_2");
    ...
    while (!viewer->wasStopped ())
    {
        ...
        inputCloudI = pointProcessorI_ptr->loadPcd((*streamIterator).string());
        cityBlock(viewer, *pointProcessorI_ptr, inputCloudI);
